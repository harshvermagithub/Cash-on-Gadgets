...
async function generatePosters() {
    console.log('Generating hiring posters with official brand assets & single-page A4 PDF configuration...');
    ...
    // Shared CSS styles for both posters
    const getPosterCss = (heroAccentColor = '#059669', roleAccentGradient = 'linear-gradient(135deg, #059669 0%, #10b981 100%)') => `
        @page {
            size: A4 portrait;
            margin: 0;
        }
...
<head>
    ...
    <style>
        ${getPosterCss('#059669', 'linear-gradient(135deg, #059669 0%, #10b981 100%)')}
    </style>
</head>
<body>
        ...
        <footer class="footer-cta-container">
            <div class="contact-details">
                <div class="contact-row-primary">
                    ${whatsappOfficialBadgeSvg(40)}
                    <div class="contact-numbers-block">
                        <span class="desk-subtitle">Official Recruitment Desk &amp; WhatsApp:</span>
                        <div class="phone-and-badge">
                            <span class="primary-phone-number">+91 90603 36060</span>
                            <span class="whatsapp-tag">Direct Call &amp; Chat</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="contact-grid-secondary">
                    <div class="secondary-item">
                        <span style="color: #059669; display: flex; align-items: center;">${mailIconSvg}</span>
                        <span><strong>Email CV:</strong> careers@fonzkart.in</span>
                    </div>
                    <div class="secondary-item">
                        <span style="color: #059669; display: flex; align-items: center;">${phoneIconSvg}</span>
                        <span><strong>Recruitment Desk:</strong> +91 90603 36060</span>
                    </div>
                    <div class="address-item">
                        <span style="color: #059669; margin-top: 1px; display: flex; align-items: center;">${mapPinIconSvg}</span>
                        <span><strong>Bangalore HQ:</strong> #69 8th cross Hegde Nagar, SRK Nagar Post, Bangalore - 560077</span>
                    </div>
                </div>
            </div>
...
<body>
    ...
    <div class="content-container">
        ...
        <div class="metrics-grid">
            <div class="metric-card">
                <div class="metric-label">Job Type</div>
                <div class="metric-value">Full-Time | On-Field</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Location</div>
                <div class="metric-value">Bangalore Hubs</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Experience</div>
                <div class="metric-value">0 – 2 Yrs (Freshers)</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Perks</div>
                <div class="metric-value">Fuel + Pick Commission</div>
            </div>
        </div>
        ...
        <div class="info-grid">
            ...
            <div class="info-card">
                ...
                <ul class="points-list">
                    <li>
                        <span class="check-icon">${checkIconSvg}</span>
                        <span><strong>Doorstep Pickup Visits:</strong> Visit scheduled customer doorsteps across assigned Bangalore pin-codes on your two-wheeler.</span>
                    </li>
                    <li>
                        <span class="check-icon">${checkIconSvg}</span>
                        <span><strong>Hands-on Gadget Diagnosis:</strong> Test screen touch, cameras, battery health, mic/speakers, biometrics &amp; verify phone IMEI.</span>
                    </li>
                    <li>
                        <span class="check-icon">${checkIconSvg}</span>
                        <span><strong>Physical Condition Check:</strong> Inspect cosmetic signs, scratches, and body condition using the FonzKart Rider App grading tool.</span>
                    </li>
                    <li>
                        <span class="check-icon">${checkIconSvg}</span>
    ...
    console.log('Launching Puppeteer for 300 DPI PNG and single-page A4 PDF export...');
    ...
    for (const p of posters) {
        ...
        // Exact 1-page ISO A4 PDF (595.92 x 841.92 pt) without any overflow or cuts
        const pdfPath = path.join(postersDir, `${p.name}.pdf`);
        await page.pdf({
            path: pdfPath,
            format: 'A4',
            scale: 793.7007 / 1200, // 0.66141725: scales 1200px -> 793.7px (210mm A4 width)
            printBackground: true,
            margin: { top: 0, right: 0, bottom: 0, left: 0 }
        });
        console.log(`Generated single-page A4 PDF: ${pdfPath}`);
    }

    await browser.close();
    console.log('All hiring posters generated successfully with 100% verified single-page A4 PDF output!');
}
