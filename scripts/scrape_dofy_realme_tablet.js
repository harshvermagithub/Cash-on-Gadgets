...
async function scrapeTablets() {
    ...
    for (const m of models) {
        ...
        try {
            ...
            const imgPath = m.ThumbnailPath ? `/models/realme-tablet/${m.ThumbnailPath.split('/').pop()}` : '';
            ...
            for (const v of variants) {
                ...
                tsv += `Realme\t${name}\t${variantName}\t${price}\t${imgPath}\n`;
            }
    ...
    fs.writeFileSync('realme_tablets.tsv', tsv);
    console.log("Written to realme_tablets.tsv");
}
